...
link:#Melissa_Joan_Hart[Melissa Joan Hart
(MJH)] is considered the patron saint of the PC revolution given her role in it, and having featured in the "Phillips Screwdriver" commercial. 
...
The GNU project made some other smart decisions, such as keeping the C code 32-bit-only rather than 16-bit C compatible. 
...
For a time, Intel/AMD were notorious for being buggy ( "`They are junk,
but cheap junk.`" ) 
